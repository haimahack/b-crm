 ...
 *@date: 2018年2月26日 下午3:39:00
 */
public class AESUtil {  
  
    //偏移量
    public static final String VIPARA = "EveryOne5293270";   //AES 为16bytes. DES 为8bytes  
    //编码方式
...
//    public static final String AES_TYPE = "AES/ECB/NoPadding";
    ...
    //私钥  
    private static final String AES_KEY="[#5+cHF%c3*l=~d4";   //AES固定格式为128/192/256 bits.即：16/24/32bytes。DES固定格式为128bits，即8bytes。  
    //字符补全
     ...
     */
    private static void test(String content) throws UnsupportedEncodingException{
    	System.out.println("加密内容：" + content);
    	//字节数
    	int num = content.getBytes(ENCODING).length; 
    ...
    private static String completionCodeFor16Bytes(String str) throws UnsupportedEncodingException{
    	int num = str.getBytes(ENCODING).length;
    	int index = num%16;
